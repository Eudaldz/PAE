...
uint32_t temps = 50000; //temps inicial de retard de transició de la seqüència dels LEDS del port 7.
uint32_t augmentTemps = 50000; //constant per augmentar el temps de transició dels leds.
...
{
   volatile uint32_t i;
   for(i = 0; i <= temps; i++); //bucle tonto para perder tiempo.
}
