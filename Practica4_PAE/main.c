...
DataPacket getPacket_MoveServo(byte id, uint16_t speed, byte sentido){
    ...
    dp.parametros[2] = (byte)(speed>>8);
    return dp;
}

/*void Moving_Speed(byte id, uint16_t vel, byte sentido){
    parametros[0] = 32;
    parametros[1] = (byte) vel;
    parametros[2] = (byte) (sentido | (vel>>8));
    TxPacket(id,3,3,parametros);
    delay_t(100);
}*/

/*void Moving_Backward(void){
    Moving_Speed(MOTOR_RIGHT, velocidad_right, ANTIHORARI);
    Moving_Speed(MOTOR_LEFT, velocidad_left, HORARI);
}

void Moving_Forward(void){
    Moving_Speed(MOTOR_RIGHT, velocidad_right, HORARI);
    Moving_Speed(MOTOR_LEFT, velocidad_left, ANTIHORARI);
}

void Change_Velocity(uint16_t vel_r, uint16_t vel_l){
    velocidad_right = vel_r;
    velocidad_left = vel_l;
}
...
void main(void)
  {
    ...
    sprintf(s, "%04x", H);
    escribir(s, linea);
    DataPacket dp = getPacket_MoveServo(2, 300, 0);
    TxPacket(dp);
    DataPacket dp2 = RxPacket();
    escribirPacket(dp2);
    while(1);
}
...
void escribirPacket(DataPacket dp){
    char s[1];
